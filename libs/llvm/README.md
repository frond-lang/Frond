...
需要一个 llvm-static CI 产出的平台原生 LLVM 共享库
(release `llvm-static-21.1.8`,https://github.com/frond-lang/llvm/releases):

| 平台 | tar.gz 资产 | 库文件 |
|---|---|---|
| Windows x64 | llvm-static-x86_64-pc-windows-msvc.tar.gz | lib/LLVM-C.dll |
| Linux x64/arm64 | llvm-static-*-unknown-linux-gnu.tar.gz | lib/libLLVM.so |
| macOS arm64/x64 | llvm-static-*-apple-darwin.tar.gz | lib/libLLVM.dylib |

加载方式二选一:
- `Lib.open("path/to/libLLVM.so")` — 开发态;
