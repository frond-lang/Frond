...
/// compute_fn (350): `lib.address_of(name): u64` — raw symbol address, 0 when
/// missing or the library is closed. Feeds address-taking C APIs (e.g.
/// mbedtls_ssl_set_bio's send/recv callbacks) without a callable ForeignFn
/// indirection; pair with has_symbol when a 0 needs a diagnostic.
pub fn compute_lib_address_of(frame: &mut Frame, node: NodeId, ctx: &EvalContext) -> Value {
    use crate::value::{HeapObj, LibValue};
    read_node_inputs!(frame, node, ctx, graph, _n, inputs);
    let lib_val = force_input(frame, inputs[0]);
    let name_val = force_input(frame, inputs[1]);
    let addr = match lib_val.heap_obj() {
        Some(HeapObj::LibVal(LibValue { shared })) => {
            let name = input_str(&name_val).unwrap_or("");
            if shared.closed.load(std::sync::atomic::Ordering::SeqCst) {
                0u64
