                            ...
                            if let Some(fn_ty) = self.lookup_method_type(this_ty, name, call_span.line, call_span.column) {
                                ...
                                if let Type::Fn(_) = self.arena.get(inst_fn) {
                                    ...
                                    for i in 1..n {
                                        ...
                                        self.unify_call_arg(params[i], arg_ty, sp.line, sp.column, matches!(&ast.expr(args[i - 1]).node, crate::ast::Ast::Expr::NullLit));
                                    }
    ...
    fn infer_super_method_call(
        ...
        if let Type::Fn(_) = self.arena.get(inst_fn) {
            ...
            for i in 1..n {
                ...
                let sp = ast.expr(args[i - 1]).span;
                self.unify_call_arg(param_types[i], arg_ty, sp.line, sp.column, matches!(&ast.expr(args[i - 1]).node, crate::ast::Ast::Expr::NullLit));
            }
            if let Some(exp) = expected {
                self.unify_or_constrain(ret_ty, exp);
            }
