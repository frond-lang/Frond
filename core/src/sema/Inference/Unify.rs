...
impl<'a> InferContext<'a> {
    ...
    pub fn unify_return_type(
        ...
        if let (Type::Async(_), Type::Async(_)) = (declared_ty, inferred_ty) {
            ...
            return self.unify_return_type(da, ia, line, column);
        }
        ...
        if let Type::Async(_) = declared_ty {
            let da = self.arena.async_value(r_declared);
            return self.unify_return_type(da, r_inferred, line, column);
        }
        ...
        match declared_ty {
            ...
            _ => {
                // Unwrap-leak guard: a Nullable/Throw body value flowing into a
                // plain declared return type compiles via the symmetric widening
                // rules but leaks null/Err to callers. The caller of
                // unify_return_type only requeues on Err, so report here.
                // Exception: `Throw<void, E>` carries no value payload — a void
                // function whose tail calls a throwing function is an
                // established idiom and stays legal.
                if let Some(kind) = self.unwrap_leak_kind(r_declared, r_inferred) {
                    let void_payload = kind == "throw" && {
                        let (vt, _) = self.arena.throw_parts(r_inferred);
                        matches!(
                            self.arena.get(self.arena.resolve(vt)),
                            crate::types::Ty::Type::Void
                        )
                    };
                    if !void_payload {
                        self.add_error_at(
                            &format!(
                                "return type mismatch: body evaluates to a {} value ('{}') but the declared return type is '{}' — unwrap explicitly ('?? default', '!', '?', or an '!= null' narrowed branch)",
                                kind,
                                self.arena.display(r_inferred),
                                self.arena.display(r_declared)
                            ),
                            line,
                            column,
                        );
    ...
    /// Detects a silent unwrap leak: `value` carries an explicit Nullable/Throw
    /// wrapper while `target` does not. Flowing such a value into a plain slot
    /// compiles (try_widen_unify's symmetric "compatible with" rules) but the
    /// null/Err payload survives to runtime, where arithmetic reads it as 0
    /// (`null + 1 == 1`) and a leaked `Ok(100)` loses its payload entirely.
    /// Unwrapping must be explicit: `?? default`, `!`, `?`, or an `!= null`
    /// flow-narrowed branch. Returns the wrapper kind on leak.
    pub fn unwrap_leak_kind(&self, target: TypeHandle, value: TypeHandle) -> Option<&'static str> {
        use crate::types::Ty::Type;
        let rt = self.arena.resolve(target);
        let rv = self.arena.resolve(value);
        match (self.arena.get(rt), self.arena.get(rv)) {
            // Target wrapped too (or inference incomplete): let the normal
            // symmetric inner-unify path decide.
            (Type::Nullable(_), _) | (Type::TypeVar(_), _) | (_, Type::TypeVar(_)) => None,
            (_, Type::Nullable(_)) => Some("nullable"),
            (Type::Throw(_), _) => None,
            (_, Type::Throw(_)) => Some("throw"),
            _ => None,
        }
    }

    pub fn unify_call_arg(
        &mut self,
        param: TypeHandle,
        arg: TypeHandle,
        line: u32,
        column: u32,
        arg_is_null_lit: bool,
    ) {
        ...
        if concrete_pair && !both_numeric && !param_is_error_iface && hard_fail {
            // Unwrap-leak guard (the REVERSE of the widening below): a value
            // wrapped in Nullable/Throw flowing into a plain parameter would
            // compile via try_widen_unify's symmetric rules but leak its
            // null/Err payload to runtime (null + 1 == 1; Ok(100) reads as 0).
            // Null LITERALS are exempt: `pick(5, null)` participates in
            // generic null-join (Bug #159) — the Nullable<tv> shape comes
            // from the expected hint, not from a leaking value.
            if let Some(kind) = self.unwrap_leak_kind(param, arg).filter(|_| !arg_is_null_lit) {
                self.add_error_at(
                    &format!(
                        "argument type incompatible: cannot pass a {} value where '{}' is expected — unwrap explicitly ('?? default', '!', '?', or an '!= null' narrowed branch)",
                        kind,
                        self.arena.display(self.arena.resolve(param))
                    ),
                    line,
                    column,
                );
