...
impl<'a> InferContext<'a> {
    ...
    pub(super) fn check_local_decl(
        ...
        let bind_ty = if let Some(ta) = type_annotation {
            ...
            // Unwrap-leak guard: a Nullable/Throw value bound to a plain
            // annotation would compile but leak null/Err into the slot
            // (the runtime then reads it as 0). Unwrapping must be explicit.
            if let Some(kind) = self.unwrap_leak_kind(annot_ty, val_ty) {
                let annot_str = self.display_type_annotation(ta, ast, annot_ty);
                let span = ast.ty(ta).span;
                self.add_error_at(
                    &format!(
                        "type annotation mismatch: cannot bind a {} value ('{}') to '{}' — unwrap explicitly ('?? default', '!', '?', or an '!= null' narrowed branch)",
                        kind,
                        self.arena.display(val_ty),
                        annot_str
                    ),
                    span.line,
                    span.column,
                );
            } else if self.try_widen_unify(annot_ty, val_ty).is_err() {
            ...
            Stmt::Return { value } => {
                if let Some(v) = value {
                    ...
                    if let Some(fn_ret) = self.expected_return {
                        let ret_span = ast.stmt(stmt).span;
                        if self
                            .unify_return_type(fn_ret, val_ty, ret_span.line, ret_span.column)
                            .is_err()
                        {
